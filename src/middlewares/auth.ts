...
export const authenticate = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // express가 자동을로 Authorization 키에 존재하는 값을 authorization 프로퍼티에 담아줌
        const authHeader = req.headers.authorization;

        // 2. 그 Authorization의 값이 'Bearer 뫄뫄뫄'로 들어오는지, 그 뫄뫄뫄가 있으면 그 값을 받아옴
        // authorization 프로퍼티의 값 타입은 string이기 때문에 startsWith() 메서드를 사용해서 확인
        ...
        if (error instanceof jwt.JsonWebTokenError) {
            res.status(401).json({
                message: "유효하지 않은 토근 형식입니다. 다시 로그인해주세요.",
            });
            return;
        }
        console.log(error);
...
export const requiredAdmin = async (req: AuthRequest, res: Response, next: NextFunction) => {
    // 그렇게 확인해온 user 정보 중, user.role === "ADMIN"인지만 판별하는 기능만 탑재
    // 이걸 구현하기 위해서 user 정보를 꺼내야 되는데, 지금 당장은 가져올 곳이 없음
    // requiredAdmin이라는 함수는 다른 경로에는 접근이 불가능하지만, 매개변수로 들어오는 req, res, next는 쓸 수 있음
    // 근데 res는 밖으로 나갈 박스니까 여기는 낙서 X
    // next는 다음으로 진행하는 기능(메서드)라 당연히 X
    // req는 외부에서 들어오는 내용이 담기는 박스지만, 낙서 O
    // 그럼. authenticate를 할 때, 사용자 경로(user)를 req에 넣자
    ...
    if (req.user.role !== RoleType.ADMIN) {
        res.status(401).json({ message: "해당 기능에 접근할 수 있는 관리자 권한이 없습니다." });
        return;
    }
    next();
};
