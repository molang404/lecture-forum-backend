...
const getInquiryById = async (req: AuthRequest<{ inquiryId: string }>, res: Response) => {
    try {
        ...
        console.log(error);
        res.status(500).json({
            message: "문의글 조회 중 서버 오류가 발생했습니다.",
        });
    }
};
...
const createInquiry = async (req: AuthRequest, res: Response) => {
    try {
        ...
        console.log(error);
        res.status(500).json({
            message: "문의글 등록 중 서버 오류가 발생했습니다.",
        });
    }
};
...
const deleteInquiry = async (req: AuthRequest<{ inquiryId: string }>, res: Response) => {
    try {
        ...
        // 컨트롤러가 중심이 돼서 정보를 마련하는 것 뿐만 아니라
        // 서비스를 조각내서 서비스를 조합해 처리
        // 그렇게 만들어진 정보들을 response에 태워 보내 에러 처리
...
// 두괄식 : 주요 문장을 위해 배치하고, 그에 대한 설명을 아래에다 배치 -> controller 중심
// 미괄식 : 주요 문장을 ~~~~해서 이렇습니다. 결과 -> service 중심
